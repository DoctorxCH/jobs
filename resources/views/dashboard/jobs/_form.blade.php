...
<div class="space-y-6">
    <div class="grid gap-4 md:grid-cols-2">
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">Title</label>
            <input type="text" name="title" value="{{ old('title', $job?->title) }}" class="mt-2 w-full pixel-outline px-3 py-2" required>
        </div>
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">SKNICE Position</label>
    ...
    <div>
        <label class="text-xs uppercase tracking-[0.2em]">Description</label>
        <textarea name="description" rows="6" class="mt-2 w-full pixel-outline px-3 py-2" required>{{ old('description', $job?->description) }}</textarea>
    </div>

    <div class="grid gap-4 md:grid-cols-3">
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">Employment type</label>
        ...
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">Workload min (%)</label>
        ...
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">Workload max (%)</label>
    ...
    <div class="grid gap-4 md:grid-cols-3">
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">Country</label>
        ...
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">Region</label>
        ...
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">City</label>
    ...
    <div class="flex flex-wrap gap-4">
        <label class="flex items-center gap-2 text-xs uppercase tracking-[0.2em]"><input type="checkbox" name="is_remote" value="1" @checked(old('is_remote', $job?->is_remote))> Remote</label>
        <label class="flex items-center gap-2 text-xs uppercase tracking-[0.2em]"><input type="checkbox" name="is_hybrid" value="1" @checked(old('is_hybrid', $job?->is_hybrid))> Hybrid</label>
        <label class="flex items-center gap-2 text-xs uppercase tracking-[0.2em]"><input type="checkbox" name="travel_required" value="1" @checked(old('travel_required', $job?->travel_required))> Travel required</label>
        <label class="flex items-center gap-2 text-xs uppercase tracking-[0.2em]"><input type="checkbox" name="has_company_car" value="1" @checked(old('has_company_car', $job?->has_company_car))> Company car</label>
    </div>

    <div class="grid gap-4 md:grid-cols-3">
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">Available from</label>
        ...
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">Application deadline</label>
        ...
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">Open positions</label>
    ...
    <div class="grid gap-4 md:grid-cols-4">
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">Salary min (gross/month)</label>
        ...
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">Salary max (gross/month)</label>
        ...
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">Currency</label>
        ...
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">Salary months</label>
    ...
    <div>
        <label class="text-xs uppercase tracking-[0.2em]">Salary note</label>
        <input type="text" name="salary_note" value="{{ old('salary_note', $job?->salary_note) }}" class="mt-2 w-full pixel-outline px-3 py-2">
    </div>

    <div class="grid gap-4 md:grid-cols-3">
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">Education level</label>
        ...
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">Min years experience</label>
    ...
    <div class="flex flex-wrap gap-4">
        <label class="flex items-center gap-2 text-xs uppercase tracking-[0.2em]"><input type="checkbox" name="is_for_graduates" value="1" @checked(old('is_for_graduates', $job?->is_for_graduates))> For graduates</label>
        <label class="flex items-center gap-2 text-xs uppercase tracking-[0.2em]"><input type="checkbox" name="is_for_disabled" value="1" @checked(old('is_for_disabled', $job?->is_for_disabled))> For disabled candidates</label>
    </div>

    <div>
        <label class="text-xs uppercase tracking-[0.2em]">Candidate note</label>
    ...
    <div>
        <label class="text-xs uppercase tracking-[0.2em]">Employer reference</label>
    ...
    <div>
        <label class="text-xs uppercase tracking-[0.2em]">HR team member</label>
    ...
    <div class="grid gap-4 md:grid-cols-2">
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">HR email</label>
            <input type="email" name="hr_email" value="{{ old('hr_email', $job?->hr_email) }}" class="mt-2 w-full pixel-outline px-3 py-2">
        </div>
        <div>
            <label class="text-xs uppercase tracking-[0.2em]">HR phone</label>
            <input type="text" name="hr_phone" value="{{ old('hr_phone', $job?->hr_phone) }}" class="mt-2 w-full pixel-outline px-3 py-2">
        </div>
    </div>

    <div>
        <label class="text-xs uppercase tracking-[0.2em]">Benefits</label>
        <select name="benefits[]" class="mt-2 w-full pixel-outline px-3 py-2" multiple>
            @foreach ($benefits as $benefit)
                <option value="{{ $benefit->id }}" @selected(in_array($benefit->id, $selectedBenefits, true))>{{ $benefit->label }}</option>
            @endforeach
        </select>
    </div>

    <div>
        <label class="text-xs uppercase tracking-[0.2em]">Driving license categories</label>
        <select name="driving_license_categories[]" class="mt-2 w-full pixel-outline px-3 py-2" multiple>
            @foreach ($drivingLicenseCategories as $license)
                <option value="{{ $license->id }}" @selected(in_array($license->id, $selectedLicenses, true))>{{ $license->code }} - {{ $license->label }}</option>
            @endforeach
        </select>
    </div>

    <div>
        <label class="text-xs uppercase tracking-[0.2em]">Languages</label>
        <div id="language-rows" class="mt-2 space-y-2">
            @foreach ($languageRows as $index => $row)
                <div class="grid gap-2 md:grid-cols-[1fr_1fr_auto]">
                    <select name="job_languages[{{ $index }}][language_code]" class="pixel-outline px-3 py-2">
                        <option value="">Language</option>
                        @foreach ($languageOptions as $code => $label)
                            <option value="{{ $code }}" @selected(($row['language_code'] ?? '') === $code)>{{ $label }}</option>
                        @endforeach
                    </select>
                    <select name="job_languages[{{ $index }}][level]" class="pixel-outline px-3 py-2">
        ...
        <button type="button" id="add-language" class="mt-2 pixel-outline px-4 py-2 text-xs uppercase tracking-[0.2em]">Add language</button>
    </div>

    <div>
        <label class="text-xs uppercase tracking-[0.2em]">Skills</label>
        <div id="skill-rows" class="mt-2 space-y-2">
            @foreach ($skillRows as $index => $row)
                <div class="grid gap-2 md:grid-cols-[1fr_1fr_auto]">
                    <select name="job_skills[{{ $index }}][skill_id]" class="pixel-outline px-3 py-2">
                        <option value="">Skill</option>
                        @foreach ($skills as $skill)
                            <option value="{{ $skill->id }}" @selected((string) ($row['skill_id'] ?? '') === (string) $skill->id)>{{ $skill->name }}</option>
                        @endforeach
                    </select>
                    <select name="job_skills[{{ $index }}][level]" class="pixel-outline px-3 py-2">
                        <option value="">Level</option>
        ...
        <button type="button" id="add-skill" class="mt-2 pixel-outline px-4 py-2 text-xs uppercase tracking-[0.2em]">Add skill</button>
    </div>
</div>
