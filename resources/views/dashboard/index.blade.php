...
<div class="space-y-6">
    ...
    <div class="flex items-start justify-between gap-6">
        <div>
            <div class="text-[10px] uppercase tracking-[0.28em] text-slate-500">Dashboard</div>
            <h1 class="mt-2 text-2xl font-bold">Hello {{ $user->name ?? auth()->user()->name }}</h1>
            ...
            @if($c)
                <div class="mt-2 text-sm text-slate-600">
                    ...
                    @endif
                    <span class="mx-2 text-slate-300">•</span>
                    Verified: <span class="font-semibold">{{ $c->verified_at ? 'YES' : 'NO' }}</span>
                </div>
            @else
                <div class="mt-2 text-sm text-slate-600">No company assigned.</div>
            @endif
        </div>
        ...
        <div class="px-3 py-2 text-xs uppercase tracking-[0.2em]">
            ...
            @else
                <div class="text-sm text-slate-600">Upload your logo in profile settings</div>
            @endif        </div>
        </div>
    ...
    <div class="grid gap-3 md:grid-cols-4">
        <div class="pixel-outline p-4">
            <div class="text-[10px] uppercase tracking-[0.28em] text-slate-500">Credits</div>
        ...
        <div class="pixel-outline p-4">
            <div class="text-[10px] uppercase tracking-[0.28em] text-slate-500">Jobs</div>
            <div class="mt-2 text-xl font-bold">{{ (int) $jobCounts['active'] }}</div>
            <div class="mt-1 text-xs text-slate-600 uppercase tracking-[0.2em]">
                {{ (int) $jobCounts['draft'] }} draft · {{ (int) $jobCounts['expired'] }} expired
            </div>
        </div>

        <div class="pixel-outline p-4">
            <div class="text-[10px] uppercase tracking-[0.28em] text-slate-500">Invoices</div>
            <div class="mt-2 text-xl font-bold">{{ (int) ($invoiceCounts['open'] ?? 0) }}</div>
            <div class="mt-1 text-xs text-slate-600 uppercase tracking-[0.2em]">
                {{ (int) ($invoiceCounts['pending'] ?? 0) }} sync pending · {{ (int) ($invoiceCounts['overdue'] ?? 0) }} overdue
            </div>
        </div>

        <div class="pixel-outline p-4">
            <div class="text-[10px] uppercase tracking-[0.28em] text-slate-500">Service</div>
            @if($tpEnabled && $tpActive)
                <div class="mt-2 text-xl font-bold">Top Partner</div>
                <div class="mt-1 text-xs text-slate-600">@if($tpUntil) until {{ $tpUntil->format('d.m.Y') }} @else no end date @endif</div>
            @elseif($tpEnabled)
                <div class="mt-2 text-xl font-bold">Top Partner</div>
                <div class="mt-1 text-xs text-slate-600">paused</div>
            @else
                <div class="mt-2 text-xl font-bold">—</div>
                <div class="mt-1 text-xs text-slate-600">no service</div>
            @endif
        </div>
    </div>
    ...
    <div class="pixel-outline p-6">
        @if($tab === 'overview')
            <div class="grid gap-6 md:grid-cols-2">
                <div>
                    <div class="text-[10px] uppercase tracking-[0.28em] text-slate-500">ToDos</div>

                    @if(empty($todos))
                        <div class="mt-3 text-sm text-slate-600">All good.</div>
                    @else
                        <ul class="mt-3 space-y-2 text-sm">
                            @foreach(array_slice($todos, 0, 5) as $t)
                                <li class="flex items-start justify-between gap-3">
                                    <div>
                                        <div class="font-semibold">{{ $t['label'] ?? 'Todo' }}</div>
                                        @if(!empty($t['hint'])) <div class="text-xs text-slate-600">{{ $t['hint'] }}</div> @endif
                                    </div>
                                    @if(!empty($t['url']))
                                        <a href="{{ $t['url'] }}" class="pixel-outline px-3 py-2 text-xs uppercase tracking-[0.2em]">Open</a>
                                    @endif
                                </li>
                            @endforeach
                        </ul>
                ...
                <div>
                    <div class="text-[10px] uppercase tracking-[0.28em] text-slate-500">Company quick</div>
                    ...
                    @if($c)
                        <div class="mt-3 grid grid-cols-2 gap-3 text-sm">
                            ...
                            <div>
                                <div class="text-xs text-slate-600">Seats free</div>
        ...
        @elseif($tab === 'jobs')
            <div class="flex items-center justify-between gap-4">
                <div>
                    <div class="text-[10px] uppercase tracking-[0.28em] text-slate-500">Jobs</div>
                    <div class="mt-1 text-sm text-slate-600">All postings.</div>
                </div>
            ...
            <div class="mt-4 space-y-2">
                ...
                @empty
                    <div class="text-sm text-slate-600">No jobs yet.</div>
                @endforelse
            </div>
        ...
        @elseif($tab === 'invoices')
            <div class="flex items-center justify-between gap-4">
                <div>
                    <div class="text-[10px] uppercase tracking-[0.28em] text-slate-500">Invoices</div>
                    <div class="mt-1 text-sm text-slate-600">Latest invoices.</div>
                </div>
                <a href="{{ route('frontend.billing.invoices.index') }}" class="pixel-outline px-4 py-2 text-xs uppercase tracking-[0.2em]">All invoices</a>
            </div>
            ...
            <div class="mt-4 space-y-2">
                @forelse($invoiceItems as $inv)
                    <div class="flex items-start justify-between gap-4 border border-slate-200 p-3">
                        <div>
                            <div class="font-semibold">Invoice #{{ $inv->id }}</div>
                            <div class="text-xs uppercase tracking-[0.2em] text-slate-500">
                                Status: {{ $inv->status ?? '—' }}
                                @if($inv->external?->sync_status)
                                    <span class="ml-2 text-slate-400">• Sync: {{ $inv->external->sync_status }}</span>
                                @endif
                            </div>
                            <div class="text-xs text-slate-500">
                                Issued: {{ $inv->issued_at?->format('Y-m-d') ?? '—' }}
                                @if($inv->due_at) · Due: {{ $inv->due_at->format('Y-m-d') }} @endif
                            </div>
                        </div>
                        <a href="{{ route('frontend.billing.invoices.show', $inv) }}" class="pixel-outline px-3 py-2 text-xs uppercase tracking-[0.2em]">View</a>
                    </div>
                @empty
                    <div class="text-sm text-slate-600">No invoices yet.</div>
                @endforelse
            </div>
        ...
        @elseif($tab === 'team')
            <div class="flex items-center justify-between gap-4">
                <div>
                    <div class="text-[10px] uppercase tracking-[0.28em] text-slate-500">Members</div>
                    <div class="mt-1 text-sm text-slate-600">Seats free: {{ $seatsFree }} ({{ $seatsLocked }} locked / {{ $seatsPurchased }} purchased)</div>
                </div>
                <a href="{{ route('frontend.team') }}" class="pixel-outline px-4 py-2 text-xs uppercase tracking-[0.2em]">Team page</a>
            </div>
                        ...
                        <div>
                            ...
                            @if(isset($m->pivot))
                                <div class="text-xs uppercase tracking-[0.2em] text-slate-500 mt-1">
                                    Role: {{ $m->pivot->role ?? '—' }} · Status: {{ $m->pivot->status ?? '—' }}
                                </div>
                            @endif
                        </div>
                    </div>
                @empty
                    <div class="text-sm text-slate-600">No members.</div>
                @endforelse
            </div>

            @if($invitations->isNotEmpty())
                <div class="mt-6">
                    <div class="text-[10px] uppercase tracking-[0.28em] text-slate-500">Pending invitations</div>
                    <div class="mt-3 space-y-2">
                        @foreach($invitations as $inv)
                            <div class="border border-slate-200 p-3">
                                <div class="font-semibold">{{ $inv->email ?? 'Invitation' }}</div>
                                <div class="text-xs text-slate-500">Created: {{ $inv->created_at?->format('Y-m-d H:i') ?? '—' }}</div>
                            </div>
                        @endforeach
                    </div>
                </div>
        ...
        @elseif($tab === 'activity')
            <div class="flex items-center justify-between gap-4">
                <div>
                    <div class="text-[10px] uppercase tracking-[0.28em] text-slate-500">Activity</div>
                    <div class="mt-1 text-sm text-slate-600">Latest events.</div>
                </div>
            </div>

            @if($activityItems->isEmpty())
                <div class="mt-3 text-sm text-slate-600">No recent activity.</div>
            @else
                <div class="mt-4 space-y-2">
                    @foreach($activityItems as $a)
                        <a href="{{ $a['url'] ?? '#' }}" class="block border border-slate-200 p-3">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    <div class="text-sm font-semibold">{{ $a['title'] ?? 'Activity' }}</div>
